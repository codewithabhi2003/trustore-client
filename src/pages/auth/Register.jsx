...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      navigate(form.role === 'storeOwner' ? '/store-register' : '/');
    } catch (err) {
  ...
  return (
    <div className="max-w-md mx-auto px-4 py-20">
      <div className="bg-card border border-border rounded-card shadow-sm p-8">
        ...
        <p className="text-sm text-text-secondary mt-1 mb-6">Join Trustore in under a minute.</p>
        ...
        <div className="grid grid-cols-2 gap-3 mb-5">
          <button
            ...
            className={`flex flex-col items-center gap-1.5 rounded-lg border py-3 text-sm font-medium transition-colors ${
              form.role === 'customer' ? 'border-accent bg-accent-soft text-accent' : 'border-border text-text-secondary'
            }`}
          >
          ...
          <button
            type="button"
            onClick={() => setForm({ ...form, role: 'storeOwner' })}
            className={`flex flex-col items-center gap-1.5 rounded-lg border py-3 text-sm font-medium transition-colors ${
              form.role === 'storeOwner' ? 'border-accent bg-accent-soft text-accent' : 'border-border text-text-secondary'
            }`}
          >
